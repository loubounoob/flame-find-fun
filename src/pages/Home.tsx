...
export default function Home() {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from("offers").select("*").eq("status", "active").order("created_at", {
        ascending: false
      });
  ...
  const sortedOffers = user && scoredOffers.length > 0 ? filteredOffers.sort((a, b) => {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from("flames").select("offer_id");
      if (error) throw error;
  ...
  return <div className="min-h-screen bg-background pb-20">
        ...
        <div className="flex items-center justify-between px-4 pb-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative flex-1 max-w-sm ml-3">
              ...
              <input type="text" placeholder="Rechercher une activité..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2 bg-background border border-border/50 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" />
              <SearchSuggestions query={searchQuery} onSelect={suggestion => setSearchQuery(suggestion)} isVisible={!!searchQuery} />
            </div>
            ...
            <Link to="/notifications">
              <Button variant="ghost" size="icon" className="relative hover:scale-110 transition-transform duration-200">
                <Bell size={20} />
                {unreadNotifications > 0 && <span className="absolute -top-1 -right-1 bg-destructive text-destructive-foreground text-xs rounded-full h-5 w-5 flex items-center justify-center font-medium">
      ...
      <section className="p-4 space-y-4 mt-4">
        ...
        <div className="max-w-2xl mx-auto space-y-4">
          {flashOffersLoading ? <div className="text-center py-8">
            ...
            </div> : filteredFlashOffers.length > 0 ? <>
            ...
            return <PromoCard key={offer.id} id={offer.id} promotionId={offer.id} offerId={offer.id} businessUserId={offer.business_user_id} title={offer.title} business={offer.business_user_id} description={offer.description} location={offer.address || offer.location} category={offer.category} image={offer.image_url || undefined} image_urls={imageUrls.length > 0 ? imageUrls : undefined} video={offer.video_url} originalPrice={offer.original_price} promotionalPrice={offer.promotional_price} discountText={`${offer.discount_percentage}% de réduction`} endDate={offer.endDate.toISOString()} flames={flamesCounts[offer.id] || 0} latitude={offer.latitude ? parseFloat(offer.latitude.toString()) : undefined} longitude={offer.longitude ? parseFloat(offer.longitude.toString()) : undefined} />;
          })}
      ...
      <section className="p-4 space-y-4">
        ...
        <div className="max-w-2xl mx-auto space-y-4 border-none">
          ...
          {sortedOffers.length === 0 && <div className="text-center py-8">
